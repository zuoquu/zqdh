...
$newsql = mysql_query("SELECT * from tune_LINK order by id desc limit 0,1");
if($row=mysql_fetch_array($newsql)){
$newid=$row["id"];
}


$sql = mysql_query("SELECT * from tune_LINK where display=0");
...
$ysql = mysql_query("SELECT * from tune_LINK where display=1 order by id desc");
while($row=mysql_fetch_array($ysql)){
echo "
<font color=#ff0000;>"
.$row["shortname"].
...
if(mysql_query($sql)){echo "审核通过<br><a href='/admin/admin_link.php'>返回后台</a> <a href='/index.php'>返回首页</a>";}
}elseif($mode == "del"){    
$delsql="DELETE FROM tune_LINK WHERE id='$id'";
if($id!=$newid){
mysql_query($delsql);}else{mysql_query($delsql);   $sql="insert into tune_LINK(shortname,longname,cat_id,url,conversation,instruction,display,views,outviews)values('记录','记录',' ','http://dh.zl88.net',' ',' ','$display','0','0')";
mysql_query($sql);}
